  ...
  cart: z
    .object({
      ...
      context: z
        ...
        .describe(
          "Localization hints including address_country, address_region, and postal code. Merchants may use these as a signal for pricing, availability, and currency estimates, but context is not authoritative for shipping. If omitted, the merchant falls back to geo-IP."
        )
        .optional(),
      attribution: z
        .object({
...
function createServer() {
  ...
  server.registerTool(
    "create_cart",
    {
      ...
      inputSchema: createCartInputShape
    },
    async ({ shop_domain, meta, cart }: z.infer<typeof z.object(createCartInputShape)>) => {
  ...
  server.registerTool(
    "get_cart",
    {
      ...
      inputSchema: getCartInputShape
    },
    async ({ shop_domain, meta, id }: z.infer<typeof z.object(getCartInputShape)>) => {
		const response = await fetch(`https://${shop_domain}/api/ucp/mcp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  server.registerTool(
    "update_cart",
    {
      ...
      inputSchema: updateCartInputShape
    },
    async ({ shop_domain, meta, id, cart }: z.infer<typeof z.object(updateCartInputShape)>) => {
      const response = await fetch(`https://${shop_domain}/api/ucp/mcp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  server.registerTool(
    "cancel_cart",
    {
      ...
      inputSchema: cancelCartInputShape
    },
    async ({ shop_domain, meta, id }: z.infer<typeof z.object(cancelCartInputShape)>) => {
      const response = await fetch(`https://${shop_domain}/api/ucp/mcp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
